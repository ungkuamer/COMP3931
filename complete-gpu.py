...
class BikePathEnv(gym.Env):
    ...
    def __init__(self, city_name, budget=1000, edge_cost_factor=10, use_gpu=True):
        ...
        if self.use_gpu:
            try:
                ...
                with cp.cuda.Device(0):
                    # Try a small computation to verify GPU works
                    x = cp.array([1, 2, 3])
                    cp.sum(x)
    ...
    def _calculate_connectivity_gpu(self):
        """Calculate connectivity metrics using GPU acceleration."""
        try:
            ...
            adj_gpu = cp.asarray(adj_matrix.todense())
            
            # Calculate clustering coefficient on GPU
            clustering = self._calculate_clustering_gpu(adj_gpu)
            print(f"Successfully calculated clustering: {clustering:.4f}")

            # Calculate shortest paths on GPU
            path_efficiency = self._calculate_path_efficiency_gpu(adj_gpu, subgraph)
            print(f"Successfully calculated path efficiency: {path_efficiency:.4f}")

            return clustering, path_efficiency
    ...
    def _calculate_path_efficiency_gpu(self, adj_gpu, subgraph):
        """Calculate path efficiency using GPU-accelerated Floyd-Warshall."""
        try:
            # Get edge weights
            weights = np.array([[subgraph[u][v].get('length', float('inf')) 
                               if subgraph.has_edge(u, v) else float('inf') 
            ...
            n = len(subgraph)
            dist = weights_gpu.copy()
            cp.fill_diagonal(dist, 0)
            
            # Floyd-Warshall algorithm on GPU
            for k in range(n):
                dist = cp.minimum(dist, 
                                dist[:, k:k+1] + dist[k:k+1, :])
            
            # Calculate average path length
            mask = (dist != float('inf')) & (dist != 0)
            if cp.any(mask):
                avg_path_length = float(cp.mean(dist[mask]))
            else:
                avg_path_length = float('inf')
            
            # Normalize path efficiency
            if avg_path_length == float('inf'):
                return 0.0
            else:
                return float(1.0 / (1.0 + avg_path_length/1000))
                
        except Exception as e:
            print(f"GPU path efficiency calculation failed: {e}")
            return 0.0
    ...
    def _calculate_connectivity(self):
        ...
        if self.use_gpu:
            return self._calculate_connectivity_gpu()
        else:
            try:
...
def train_rl_model(city_name='Colmar, France', budget=150000, total_timesteps=5000):
...
def main():
    ...
    parser.add_argument('--city', type=str, default='Colmar, France', 
                        help='City name to analyze (default: Colmar, France)')
    parser.add_argument('--budget', type=float, default=150000, 
                        help='Budget for bike path construction (default: 150000)')
    parser.add_argument('--timesteps', type=int, default=5000, 
                        help='Training timesteps (default: 5000)')
