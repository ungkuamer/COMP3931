...
class BikePathEnv(gym.Env):
    ...
    def _calculate_connectivity(self):
        ...
        try:
            ...
            print(f"Split into {len(chunks)} chunks of approximately {chunk_size} pairs each")
            
            with Pool(n_processes) as pool:
                # Prepare arguments for each chunk
                chunk_args = [(subgraph, chunk) for chunk in chunks]
                
                # Calculate path lengths in parallel with progress tracking
                path_lengths_chunks = []
                print("Starting parallel path length calculation...")
                for i, result in enumerate(tqdm(pool.imap(self._calculate_path_lengths_chunk, chunk_args), 
                                            total=len(chunks), 
