...
class BikePathEnv(gym.Env):
    ...
    def _calculate_node_clustering(self, args):
        """Calculate clustering coefficient for a single node with advanced optimizations."""
        G, node = args
        try:
            # Get neighbors as a set for O(1) membership testing
            neighbor_set = set(G.neighbors(node))
            n_neighbors = len(neighbor_set)
            
            # Early exit for nodes with fewer than 2 neighbors (no triangles possible)
            if n_neighbors < 2:
                return 0.0
                
            # Pre-calculate possible triangles
            possible = n_neighbors * (n_neighbors - 1) / 2
            
            # OPTIMIZATION: For very high-degree nodes, use adjacency view
            if n_neighbors > 100:
                # For high-degree nodes, use NetworkX's optimized functions
                try:
                    return nx.clustering(G, node)
                except:
                    # Fallback to manual calculation if NetworkX function fails
                    pass
            
            # OPTIMIZATION: Use set operations instead of iterating over edges
            # This is much faster for dense neighborhoods
            triangles = 0
            neighbors_list = list(neighbor_set)
            
            # OPTIMIZATION: Use vectorized approach for nodes with >= 20 neighbors
            # For larger neighborhoods, batch processing is more efficient
            if n_neighbors >= 20:
                # Process neighbors in batches for better memory locality
                batch_size = min(20, n_neighbors // 2)
                for i in range(0, n_neighbors, batch_size):
                    batch_end = min(i + batch_size, n_neighbors)
                    batch_neighbors = neighbors_list[i:batch_end]
                    
                    # For each neighbor in batch, check connections to all other neighbors
                    for n1 in batch_neighbors:
                        # Get n1's neighbors as a set for fast intersection
                        n1_neighbors = set(G.neighbors(n1)) & neighbor_set  # Only interested in common neighbors
                        triangles += len(n1_neighbors)
                
                # Each triangle is counted twice in this approach (once from each end)
                triangles //= 2
            else:
                # For smaller neighborhoods, direct iteration is more efficient
    ...
    def _calculate_path_lengths_chunk(self, args):
        """Calculate path lengths for a chunk of node pairs with optimization."""
        G, node_pairs = args
        lengths = []
        
        # OPTIMIZATION: Use a dictionary to cache paths between nodes
        # This avoids recalculating paths between the same nodes
        path_cache = {}
        
        # OPTIMIZATION: Process nodes in batches to improve memory locality
        # Group nodes by their source node to compute multiple targets at once
        source_groups = {}
        for u, v in node_pairs:
            if u not in source_groups:
                source_groups[u] = []
            source_groups[u].append(v)
        
        # Process each source node once, getting paths to all its targets
        for source, targets in source_groups.items():
            try:
                # OPTIMIZATION: Use single_source_dijkstra_path_length for multiple targets
                # This is much more efficient than calling dijkstra_path_length separately for each target
                distances = nx.single_source_dijkstra_path_length(G, source, weight='length', cutoff=None)
                
                # Extract distances to our target nodes
                for target in targets:
                    if target in distances:
                        length = distances[target]
                        
                        # Cache the result (normalized to ensure symmetry)
                        node_pair = (min(source, target), max(source, target))
                        path_cache[node_pair] = length
                        
                        if length > 0:
                            lengths.append(length)
            except Exception as e:
                # Try individual paths if the batch approach fails for a source
                for target in targets:
                    try:
                        # Use cached path if available
                        node_pair = (min(source, target), max(source, target))
                        if node_pair in path_cache:
                            length = path_cache[node_pair]
                            if length > 0:
                                lengths.append(length)
                            continue
                        
                        # Fall back to individual path calculation
                        ...
                        print(f"Error calculating path length for nodes ({source}, {target})")
                        continue
    ...
    def _calculate_connectivity(self):
        """Calculate connectivity metrics using optimized algorithms, node batching and parallel processing."""
        try:
            ...
            # Convert multigraph to simple graph - more efficiently
            ...
            # Get largest connected component - more efficiently
            ...
            # OPTIMIZATION: Node batching for clustering coefficient
            # This helps with memory usage for large graphs
            node_count = len(subgraph)
            print(f"Processing clustering for {node_count} nodes")
            
            if node_count > 1000:
                # For very large graphs, use batched processing
                batch_size = min(500, node_count // n_processes)
                print(f"Using batched processing with batch size {batch_size}")
                
                # Create node batches
                all_nodes = list(subgraph.nodes())
                node_batches = [all_nodes[i:i+batch_size] for i in range(0, node_count, batch_size)]
                
                # Process clustering coefficient in batches
                clustering_values = []
                
                with Pool(n_processes) as pool:
                    # Process each batch in parallel
                    batch_args = [(subgraph, batch) for batch in node_batches]
                    batch_results = pool.map(self._calculate_clustering_batch, batch_args)
                    
                    # Combine results
                    clustering_values = list(itertools.chain.from_iterable(batch_results))
                
                # Calculate average clustering
                clustering = np.mean(clustering_values) if clustering_values else 0.0
            else:
                # For smaller graphs, use NetworkX's built-in function
                try:
                    # Calculate global clustering coefficient (transitivity)
                    clustering = nx.transitivity(subgraph)
                    
                    # If transitivity returns None or NaN (can happen with certain graph structures),
                    # fall back to average local clustering
                    if clustering is None or np.isnan(clustering):
                        # Use NetworkX's optimized implementation
                        clustering_dict = nx.clustering(subgraph)
                        clustering = np.mean(list(clustering_dict.values()))
                except Exception as cluster_err:
                    print(f"Error in clustering calculation: {cluster_err}")
                    # Fallback to manual calculation for a sample of nodes
                    sample_size = min(500, node_count)
                    sampled_nodes = random.sample(list(subgraph.nodes()), sample_size)
                    clustering_dict = {n: self._calculate_node_clustering((subgraph, n)) 
                                    for n in sampled_nodes}
                    clustering = np.mean(list(clustering_dict.values()))
            
            print(f"Successfully calculated clustering: {clustering:.4f}")
            
            # OPTIMIZATION: Improved path length calculation with node batching
            # Determine if we need to use batching based on graph size
            large_graph = node_count > 500
            
            if large_graph:
                # For large graphs, use batched approach with smart sampling
                print(f"Using batched path calculation for large graph ({node_count} nodes)")
                
                # OPTIMIZATION: Stratified sampling to ensure representation across graph
                # This samples from different regions of the graph
                # First, get a set of "seed" nodes distributed across the graph
                degree_centrality = nx.degree_centrality(subgraph)
                betweenness_sample_size = min(100, node_count // 10)
                
                # Sample some nodes with high betweenness as seeds
                high_centrality_nodes = sorted(degree_centrality.items(), 
                                            key=lambda x: x[1], 
                                            reverse=True)[:betweenness_sample_size]
                seed_nodes = [node for node, _ in high_centrality_nodes]
                
                # For each seed node, get its neighborhood as a batch
                batches = []
                for seed in seed_nodes:
                    # Get 1-hop and 2-hop neighbors
                    neighbors = set(nx.single_source_shortest_path_length(subgraph, seed, cutoff=2).keys())
                    if len(neighbors) > 50:  # If too many neighbors, sample
                        neighbors = random.sample(list(neighbors), 50)
                    batches.append(list(neighbors))
                
                # Make sure batches don't get too large
                batches = [batch[:100] for batch in batches]
                
                # Calculate path lengths within each batch
                all_path_lengths = []
                
                with Pool(n_processes) as pool:
                    # For each batch, calculate path lengths between nodes in the batch
                    batch_args = [(subgraph, list(itertools.combinations(batch, 2))) 
                                for batch in batches if len(batch) >= 2]
                    
                    # Process batches in parallel
                    batch_results = pool.map(self._calculate_path_lengths_chunk, batch_args)
                    
                    # Combine results
                    all_path_lengths = list(itertools.chain.from_iterable(batch_results))
                
                # Calculate average path length
                if all_path_lengths:
                    avg_path_length = np.mean(all_path_lengths)
                    print(f"Successfully calculated {len(all_path_lengths)} path lengths using batched approach")
                    print(f"Average path length: {avg_path_length:.1f}m")
                else:
                    # Fallback to approximate method
                    print("Warning: Batched calculation produced no path lengths, using approximation")
                    # Use an approximation based on graph diameter
                    try:
                        # Sample a subset for diameter calculation
                        sample_size = min(100, node_count)
                        sampled_nodes = random.sample(list(subgraph.nodes()), sample_size)
                        sampled_subgraph = subgraph.subgraph(sampled_nodes)
                        # Use eccentricity as approximation
                        eccentricities = nx.eccentricity(sampled_subgraph)
                        diameter = max(eccentricities.values())
                        # Approximate average path length as diameter/2
                        avg_path_length = diameter / 2
                    except Exception as approx_err:
                        print(f"Error in path approximation: {approx_err}")
                        avg_path_length = float('inf')
            else:
                # For smaller graphs, try to use NetworkX's efficient implementation
                try:
                    # This function is much more efficient than calculating paths manually
                    avg_path_length = nx.average_shortest_path_length(subgraph, weight='length')
                    print(f"Successfully calculated average path length: {avg_path_length:.1f}m")
                except nx.NetworkXError as nx_err:
                    # Handle disconnected graphs
                    print(f"Network error in path calculation: {nx_err}")
                    print("Falling back to sampling-based path length calculation")
                    
                    # Sample nodes proportional to degree for better representation
                    nodes = list(subgraph.nodes())
                    degrees = [subgraph.degree(n) for n in nodes]
                    total_degree = sum(degrees) + 0.001 * len(degrees)
                    probs = [(d + 0.001)/total_degree for d in degrees]
                    
                    sample_size = min(100, len(nodes))
                    sampled_nodes = np.random.choice(nodes, size=sample_size, replace=False, p=probs)
                    
                    # Create node pairs more efficiently
                    node_pairs = list(itertools.combinations(sampled_nodes, 2))
                    
                    # Better chunking for parallel processing
                    n_chunks = min(len(node_pairs), n_processes * 4)
                    chunk_size = max(1, len(node_pairs) // n_chunks)
                    chunks = [node_pairs[i:i + chunk_size] for i in range(0, len(node_pairs), chunk_size)]
                    
                    # Use optimized function for path calculation
                    with Pool(n_processes) as pool:
            ...
            if avg_path_length == float('inf'):
                ...
                # Improved normalization formula for better scaling
                normalized_path = 1.0 / (1.0 + avg_path_length/500)  # Adjusted scaling factor
        ...
        # New helper method for clustering batch processing

    def _calculate_clustering_batch(self, args):
        ...
        for node in node_batch:
            try:
                ...
                # Calculate local clustering coefficient more efficiently
                # Use set operations instead of iterating over all combinations
                neighbor_set = set(neighbors)
                edge_count = 0
                
                # Count edges between neighbors - optimized algorithm
                for i, neighbor1 in enumerate(neighbors[:-1]):
                    # Get second neighbors that come after current neighbor in the list
                    # This avoids counting edges twice
                ...
                clustering = edge_count / possible_edges if possible_edges > 0 else 0.0
