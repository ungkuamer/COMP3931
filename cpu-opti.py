...
class BikePathEnv(gym.Env):
    ...
    def _calculate_path_lengths_chunk(self, args):
        ...
        for u, v in node_pairs:
            try:
                ...
                node_pair = (min(u, v), max(u, v))  # Normalize order to improve cache hits
                ...
                # OPTIMIZATION: Use more efficient bidirectional Dijkstra for path calculation
                length = nx.dijkstra_path_length(G, u, v, weight='length')
                ...
                # Silently ignore path calculation failures
                continue
    ...
    def _calculate_connectivity(self):
        """Calculate connectivity metrics using optimized algorithms and parallel processing."""
        try:
            ...
            # OPTIMIZATION: Use NetworkX's built-in clustering functions for better performance
            ...
            # OPTIMIZATION: Improved path length calculation
            if len(subgraph) > 200:
                # For large graphs, use a smarter sampling approach
                # OPTIMIZATION: Sample nodes proportional to their degree for better representation
                nodes = list(subgraph.nodes())
                degrees = [subgraph.degree(n) for n in nodes]
                # Add a small constant to prevent zero probability for isolated nodes
                ...
                sampled_nodes = np.random.choice(nodes, 
                                            size=min(100, len(nodes)), 
                                            replace=False, 
                                            p=probs)
            else:
                # For smaller graphs, we can use all nodes
                sampled_nodes = list(subgraph.nodes())
            
            # OPTIMIZATION: Use a more efficient algorithm for average path length
            # Instead of calculating all pairs, use NetworkX's average_shortest_path_length
            try:
                ...
                # OPTIMIZATION: Better chunking for parallel processing
                ...
                # OPTIMIZATION: Use a more optimized function for path calculation
            ...
            if avg_path_length == float('inf'):
                ...
                # OPTIMIZATION: Improved normalization formula for better scaling
